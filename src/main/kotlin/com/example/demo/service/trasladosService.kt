...
class trasladosService (
    private val repositorioTraslados: repositorioTraslados,
    private val repositorioDetallesTraslados: repositorioDetallesTraslados
) {
    ...
    fun update(detallesTraslado: detallesTraslado):detallesTraslado {
        return if(detallesTraslado.IDDetallesTraslado!=null&&repositorioDetallesTraslados.existsById(detallesTraslado.IDDetallesTraslado!!.toString())){ repositorioDetallesTraslados.save(detallesTraslado) }
        else{ throw IllegalArgumentException("detalle de traslado no encontrado") }
    }
}